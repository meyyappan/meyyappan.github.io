...
The issue: the motherboard wasn't allocating memory address space to the discrete GPU. Specifically, the BAR (Base Address Register) allocation that lets the CPU and OS actually talk to the card. This is a known issue with small form factor and business-class machines that weren't designed with discrete GPUs in mind.
